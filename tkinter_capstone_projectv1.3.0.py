...
import random
# importing some libraries to allow us to communicate with website/databases
import requests
#importing JSON to format the data that comes in
import json

image = Image.open("tic_tac_toe_O.jpeg")
image2 = Image.open("tic_tac_toe_X.png")
sun_with_cloud_image = Image.open("weather_cloudy_wsun_icon.jpeg")
...
class app_interface():
    ...
    def __init__(self, root_window):
        ...
        self.sun_with_cloud = ImageTk.PhotoImage(sun_with_cloud_image.resize((70, 60), Image.ANTIALIAS))
        self.work = tk.StringVar(self.root)
    ...
    def weather_app(self):
        ...
        some_window = tk.Toplevel()
        some_window.geometry('+500-200')

        def get_city_weather_info(city_input='Tokyo'):
            website_response_via_city = requests.get(
                f"https://api.openweathermap.org/data/2.5/weather?q={city_input}&appid=65bd657c42d91d5a3e971902e69df27c&units=imperial")
        ...
        content_frame = ttk.Frame(some_window, relief='sunken', borderwidth=10)

        location = tk.StringVar(some_window)
        location.set(f"{entire_data_via_city['name']}, {gen_info['country']}")
        temperature = tk.StringVar(some_window)
        temperature.set(f"{main_weather_info['temp']}")
        real_feel_temp = tk.StringVar(some_window)
        real_feel_temp.set(f"{main_weather_info['feels_like']}")
        description = tk.StringVar(some_window)
        description.set(f"{weather_description['description']}")
        city_name = tk.StringVar(some_window)
        ...
        weather_image = ttk.Label(image_frame, image= self.sun_with_cloud )
